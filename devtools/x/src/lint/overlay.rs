...
impl<'cfg> PackageLinter for OverlayFeatures<'cfg> {
    fn run<'l>(
        ...
        // Overlay features shouldn't be enabled by default for any package.
        let default_members = ctx.default_members();
        if let Some(features) = default_members.unified_set().features_for(package.id()) {
            for feature in &self.config.features {
                if features.contains(feature) {
                    // TODO: explain why the feature was introduced, Cargo can be mysterious
                    out.write(
                        LintLevel::Error,
                        format!("overlay feature \"{}\" present in default build", feature),
                    );
                }
            }
        }
        ...
        //
        // The checks after this point only run for members in the default build set.
        //
